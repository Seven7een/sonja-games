/**
 * WordleStats Page
 * Displays user statistics and game history for Wordle
 */

import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../../core/hooks/useAuth';
import { getUserStats, getGameHistory } from '../services/wordleApi';
import { WordleStats as WordleStatsType, GameHistoryItem } from '../types/wordle.types';
import StatsPanel from '../components/StatsPanel';

const WordleStats: React.FC = () => {
  const { isAuthenticated, isLoading: authLoading } = useAuth();
  const navigate = useNavigate();

  const [stats, setStats] = useState<WordleStatsType | null>(null);
                ...
                {history.map((game) => (
                  ...
                  >
                    ...
                    <div className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      {game.won ? `${game.attempts_used}/6 guesses` : '6/6 guesses'}
                    </div>
                  </div>
                ))}
              </div>
